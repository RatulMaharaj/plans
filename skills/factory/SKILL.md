...
3. Secrets: `CLAUDE_CODE_OAUTH_TOKEN` (`claude setup-token`),
   ...
   optionally `FACTORY_PAT` (a machine account's fine-grained PAT — Contents
   and Pull requests read/write, the account a write collaborator; without
   it, PRs are bot-authored and each needs one workflow-approval click).
   You cannot create secrets for the owner; say plainly what is missing and
...
4. Repository settings: Actions must be allowed to create and approve pull
   ...
   ruleset requiring pull requests will block the claim flip — the skill's
   unclaimed mode covers it, but say so; `deletion` and `non_fast_forward`
   rules cost the factory nothing.
