...
export type ShareLink = {
  id: string;
  /** The file in the workspace this link opens. */
  path: string;
  createdBy: string;
  createdAt: number;
  expiresAt: number;
...
export const workspace = {
  ...
  readUrl: (id: string, path = "plan.md") => `${serverUrl()}/w/${id}/${path}`,

  /**
   * Share links: a token each, listed and revocable, for readers with no
   * account. A link names one file; the token is still the workspace's, so
   * `revokeAll` is what copying a plan out of the room costs. Minting, listing
   * and revoking are member-only; the reading is the viewer's, with the token
   * from the URL's fragment.
   */
  share: {
    mint: (id: string, path: string) =>
      call<ShareLink & { token: string }>(`/workspaces/${id}/share`, { body: { path } }),
    ...
    revokeAll: (id: string) =>
      call<{ ok: true; revoked: number }>(`/workspaces/${id}/share/revoke`, { body: { all: true } }),
  },
  /**
...
export function openRoom(id: string, workspaceId: string, session: string, me: Presence): Room {
  ...
  const connect = () => {
    if (closed) return;
    const url = `${serverUrl().replace(/^http/, "ws")}/ws/${id}?token=${encodeURIComponent(
      session,
