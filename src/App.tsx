...
export default function App() {
  ...
  /** The workspace and file whose share links are open, if any. */
  const [wsSharing, setWsSharing] = useState<null | { id: string; path: string }>(null);
  ...
  /**
   * A share link to the open file, minted and on the clipboard in one
   * gesture: "look at this" should not cost a detour through a management
   * page. The links this makes are listed and revocable in the sheet behind
   * the same control.
   */
   ...
   *
   * The workspace's share links are revoked on the way out. A link is a
   * capability into a document that is now somewhere else, and the moment
  ...
  const copyWorkspaceOut = useCallback(
    async (id: string, path: string, repoPath: string, relPath: string) => {
      ...
      try {
        ...
        const killed = await workspace.share
          .revokeAll(id)
          .then((r) => r.revoked)
          .catch(() => 0);
        await refreshFiles();
        ...
        notify(
          killed > 0
            ? `Copied to ${relPath} — ${killed} share link${killed > 1 ? "s" : ""} revoked`
            : `Copied to ${relPath}`,
        );
      } catch (e) {
        notify(String(e), "error");
  ...
  const activeWsRoom = useMemo(() => {
    ...
    const docId = (wsTrees[id] ?? []).find((e) => e.path === wsFileOf(activePath))?.doc;
  ...
  return (
    ...
    >
                      ...
                      (() => {
                        ...
                        return (
                          <>
                            ...
                            <button
                              className="rail-btn"
                              onClick={() => setWsSharing({ id, path: file })}
                              title="A read-only link to this file, for someone with no account"
                            >
                              Share…
                            </button>
                            <button
      ...
      {wsSharing && (
        <ShareSheet
          id={wsSharing.id}
          name={workspaces.find((w) => w.id === wsSharing.id)?.name ?? "this workspace"}
          path={wsSharing.path}
          notify={notify}
      ...
      <Palette
        ...
        onCopyShareLink={(id) => void copyShareLink(id, wsFileOf(activePath))}
        onShareLinks={(id) => setWsSharing({ id, path: wsFileOf(activePath) })}
        chats={chats}
        onNewChat={newChat}
        onOpenChat={openChat}
        onDeleteChat={(id) => void deleteChat(id)}
